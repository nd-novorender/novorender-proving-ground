...
mediump vec3 getIBLRadianceGGX(mediump vec3 n, vec3 v, mediump float perceptualRoughness, mediump vec3 specularColor) {\r
    ...
    mediump vec4 specularSample = textureLod(textures.ibl.specular, camera.localBackgroundMatrixNormal * reflection, lod);\r
...
mediump vec3 getIBLRadianceLambertian(mediump vec3 n, mediump vec3 diffuseColor) {\r
    vec3 diffuseLight = texture(textures.ibl.diffuse, camera.localBackgroundMatrixNormal * n).rgb;\r
...
\r
const float highLightsTextureRows = 8.;\r
\r
// Get normal, tangent and bitangent vectors.\r
...
\r
    ...
    highp vec4 textureInfo0 = vec4(-1);\r
    highp vec4 textureInfo1 = vec4(-1);\r
    ...
    if(highlight != 0U || scene.applyDefaultHighlight) {\r
        ...
        textureInfo0 = texture(textures.highlights, vec2(u, 5.5 / highLightsTextureRows));\r
        textureInfo1 = texture(textures.highlights, vec2(u, 6.5 / highLightsTextureRows));\r
...
\r
#if defined (PBR)\r
        float array_index = textureInfo0.r;\r
        // float array_index = float(highlight) - 2.;\r
        if(array_index >= 0.) {\r
            mediump mat2 uvMat = mat2(textureInfo0.gb, vec2(-textureInfo0.b, textureInfo0.g));\r
...
\r
            ...
            MaterialInfo materialInfo = getMaterialInfo(baseColor.rgb, norSample.z, norSample.w, textureInfo0.a);\r
\r
            // LIGHTING\r
            ...
            mediump vec3 f_ambient = vec3(1);\r
\r
            mediump vec3 color = mix(f_diffuse + f_specular, f_ambient, textureInfo1.a);\r
...
\r
            mediump vec3 irradiance = texture(textures.ibl.diffuse, camera.localBackgroundMatrixNormal * N).rgb * perceptualRoughness;\r
            mediump float lod = perceptualRoughness * (scene.iblMipCount - 1.0);\r
            mediump vec3 reflection = textureLod(textures.ibl.specular, camera.localBackgroundMatrixNormal * reflect(V, N), lod).rgb * (1. - perceptualRoughness);\r
\r
